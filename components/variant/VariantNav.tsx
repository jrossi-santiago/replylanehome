    ...
    <header className="sticky top-0 z-50 border-b border-paper/8 bg-ink/90 backdrop-blur-md">
      ...
      >
        ...
        >
          ...
          <span className="font-head text-[28px] leading-none tracking-[0.04em] text-paper">
            replylane
          </span>
      ...
      {open ? (
        ...
        >
          ...
          <div className="page-container flex flex-col gap-5 pt-10">
            {variant.nav.links.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="font-head text-[36px] leading-none text-paper no-underline"
                onClick={() => setOpen(false)}
              >
                {l.label}
              </a>
