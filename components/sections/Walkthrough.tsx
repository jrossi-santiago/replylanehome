...
export function Walkthrough() {
  return (
    <Section surface="paper">
      ...
      <h2 className="type-h2 mt-3 max-w-2xl text-ink">{walkthrough.h2}</h2>
        ...
        {walkthrough.steps.map((step) => (
          <li key={step.n}>
            <p className="font-head text-[18px] font-semibold text-ink">
              <span className="font-mono text-[13px] font-normal text-ink/45">{step.n} · </span>
              {step.title}
            </p>
            {"body" in step && step.body ? (
              <p className="mt-3 font-body text-[16px] leading-[25px] text-ink/75">{step.body}</p>
            ) : null}
            {"post" in step && step.post ? (
              <blockquote className="mt-4 rounded-xl bg-snow p-5 shadow-card">
                <p className="font-body text-[15px] leading-[22px] text-ink">{step.post}</p>
              </blockquote>
            ) : null}
            ...
            {"draft" in step && step.draft ? (
              <div className="mt-4">
                <Draft text={step.draft} count={askDraftLength} actions={false} />
              </div>
            ) : null}
          </li>
        ))}
